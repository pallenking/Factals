...
class WorldModel : Atom {
	...
	var eventLimit				= 0		// stop if eventNow would be > eventLimit
	 ...
	 // Events in Hash
    var event : Event?			= nil	// to get event data
	...
	override init(_ config:FwConfig = [:]) {
		...
		if let ev				= Event(any:localConfig["events"]) {
			localConfig["events"] = nil
			event				= ev
		}
		if let eli				= localConfig["eventLimit"]?.asInt {
			eventLimit = eli
		}
		prob					= nil
		if let f				= localConfig["prob"]?.asFloat {
//		if let str				= localConfig["prob"],
//		  let f					= str.asFloat {
			prob				= f
		}
	}
	...
	required init(from decoder: Decoder) throws {
		...
		delayedEvents	= try container.decode(	   [Event].self, forKey:.delayedEvents)
	...
	override func copy(with zone: NSZone?=nil) -> Any {
	 ...
	 // MARK: - 3.7 Equitable
	func varsOfWorldModelEq(_ rhs:Part) -> Bool {
		guard let rhsAsWorldModel = rhs as? WorldModel else {	return false	}
		return   timingChain == rhsAsWorldModel.timingChain
			&& delayedEvents == rhsAsWorldModel.delayedEvents
			&& eventNow 	 == rhsAsWorldModel.eventNow
			&& 	  eventLimit == rhsAsWorldModel.eventLimit
			&& 		   event == rhsAsWorldModel.event
			&& 	  eventIndex == rhsAsWorldModel.eventIndex
			&& 			prob == rhsAsWorldModel.prob
	}
	override func equalsPart(_ part:Part) -> Bool {
		return super.equalsPart(part) && varsOfWorldModelEq(part)
	}
	...
	func addDelayed(event:Event, asyncData:Bool, eventDownPause:Bool) {
		delayedEvents?.insert(event, at:0)
	}

	 // return next event generated by WorldModel, and it's simple methods
	func dequeEvent() -> Event? {

		 // Delayed Events have been stored here if timing chain was busy
		if let rv:Event	= delayedEvents?.popLast() {
			return rv
		}

		guard eventLimit < 0 || eventLimit > eventNow else { return nil			}
		guard event != nil 						else {	return nil	/* space in event */}
		guard case .anArray(let array)? = event else {	fatalError("Need an Event array here!")	}
		guard eventIndex < array.count			else {	return nil				}

		let rv 					= array[eventIndex]
		eventIndex				+= 1		// get next Event
											// Event signals 'again'
		 // Loop
		if case let Event.aString(s) = rv, s == "again" {
			assert(eventIndex > 1, "'Again' cannot be first element of Event array")
