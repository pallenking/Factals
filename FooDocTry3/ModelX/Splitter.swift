...
class Splitter : Atom {
 ...
 //https://stackoverflow.com/questions/24561490/swift-protocol-iboutlet-property-cannot-have-non-object-type
	...
	func varsOfSplitterEq(_ rhs:Part) -> Bool {
		guard let rhsAsSplitter		= rhs as? Splitter else {	return false	}
		return onlyPosativeWinners == rhsAsSplitter.onlyPosativeWinners
			&& 		 combineWinner == rhsAsSplitter.combineWinner
			&& 			 upIsDirty == rhsAsSplitter.upIsDirty
			&&				    a1 == rhsAsSplitter.a1
	}
	override func equalsPart(_ part:Part) -> Bool {
		return	super.equalsPart(part) && varsOfSplitterEq(part)
	}
 ...
 //		}
	...
	override func port(named wantName:String, localUp wantUp:Bool?, wantOpen:Bool, allowDuplicates:Bool) -> Port? {
		...
		if (wantName == "share" ||
		    wantName == ""),			  // Want a new share
//		if wantName == "share",				 // Want a new share
