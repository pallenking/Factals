...
// 		rv["T"]					= "pc "	// tPrev:	Terciary Port, value at PreviousX time
	...
	override func copy(with zone: NSZone?=nil) -> Any {
		let theCopy : Previous	= super.copy(with:zone) as! Previous
	 ...
	 // MARK: - 3.7 Equitable
	func varsOfPreviousEq(_ rhs:Part) -> Bool {
		guard let rhsAsPrevious		= rhs as? Previous else {	return false	}
		return bias == rhsAsPrevious.bias
			&& majorMode	== rhsAsPrevious.majorMode
			&& minorMode	== rhsAsPrevious.minorMode
			&& src4sCur		== rhsAsPrevious.src4sCur
			&& src4tPrev	== rhsAsPrevious.src4tPrev
			&& src4lLatch	== rhsAsPrevious.src4lLatch
			&& src4pPri		== rhsAsPrevious.src4pPri
	}
	override func equalsPart(_ part:Part) -> Bool {
		return	super.equalsPart(part) && varsOfPreviousEq(part)
	}

	 // MARK: -  7. Simulator Messages
	override func receiveMessage(event:FwEvent) {
		if event.fwType == .clockPrevious {//sim_clockPreviousX {
	...
	func clockPrevious ()	{
		...
		if newVal != nil && !newVal!.isNan {
			...
			ports["L"]?.take(value:newVal!)
			 //********//
			root!.simulator.kickstart = 4	// start simulator after L port changes
		}else{
			atEve(4, logd("$$$$$$$$ " + msg))
		}
	}
...
bug;						atDat(4, logd("Mode Port: " + //%% curMode=%-->%",
						...
						root!.simulator.kickstart = 4	// start simulator after changing minor mode
					}
				}
			}
		}
