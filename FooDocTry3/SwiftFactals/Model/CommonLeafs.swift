...
enum LeafKind : String, Equatable, Codable {
...
extension Leaf {	/// Generate Common Leafs
	...
	func check4nil(_ etcHash:[FwConfig]) {	
		for hash in etcHash {
...
//			check4nil([etc3, etc4, etc5])
		case .genAtom:
			...
			self.init(of:leafKind, bindings:b, parts:p, fwConfig:etc1)
			check4nil([etc3, etc4, etc5])
// 		// -------- Generic Splitter -------------------------------------------------------
//		case .splitter:
//			self.init(of:leafKind, bindings:[:], parts:[
