...
//	override func hasPorts() -> [String:String] {	return ["P":"pcM"]	}	// like Tunnel
	...
	override func copy(with zone: NSZone?=nil) -> Any {
		let theCopy : FwBundle		= super.copy(with:zone) as! FwBundle
	 ...
	 // MARK: - 3.7 Equitable
	func varsOfFwBundleEq(_ rhs:Part) -> Bool {
		guard let rhsAsFwBundle	= rhs as? FwBundle else {	return false		}
		return true//leafStruc 	== rhsAsFwBundle.leafStruc	// Protocol 'FwAny' as a type cannot conform to 'Equatable'
			&& leafKind 	== rhsAsFwBundle.leafKind
			&& label 		== rhsAsFwBundle.label
	}
	override func equalsPart(_ part:Part) -> Bool {
		return	super.equalsPart(part) && varsOfFwBundleEq(part)
	}
