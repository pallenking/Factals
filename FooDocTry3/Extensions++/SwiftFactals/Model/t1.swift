	...
	override init(_ config:FwConfig = [:]) {
		...
		for (portName, portProp) in hasPorts( ) {	// process "c", "M" and "b"
			if portProp.contains("c") {		// c --> create at birth
				let newPort		= portProp.contains("M") ?
...
//?		proxyColor 				= try container.decode(			NSColor.self, forKey:.proxyColor)
	 ...
	 // MARK: - 3.7 Equitable
	func varsOfAtomEq(_ rhs:Part) -> Bool {
		guard let rhsAsAtom		= rhs as? Atom else {		return false		}
		return bandColor	== rhsAsAtom.bandColor
			&& proxyColor	== rhsAsAtom.proxyColor
	//		&& ports.equalsPart(rhsAsAtom.ports)
//			&& ports 		== rhsAsAtom.ports
			&& bindings		== rhsAsAtom.bindings
	}
	override func equalsPart(_ part:Part) -> Bool {
		return	super.equalsPart(part) && varsOfAtomEq(part)
	}
	...
	func port(named wantName:String?=nil, localUp wantUp:Bool?=nil, wantOpen:Bool=false, allowDuplicates:Bool=false) -> Port? {
		atBld(9, logd(" .CALLS.port(named:\"\(wantName ?? "<nil>")\" want:\(ppUp(wantUp)) wantOpen:\(wantOpen) allowDuplicates:\(allowDuplicates))"))
//		var s : Atom			= self				// Initial standing
//		var sWantUp				= wantUp			// Initial up
		...
		if let b				= bindings, 		// Atom has a :H:Binding
		 wantName != nil, 							 // Port name is defined
	 ...
	 /// Find all existing Ports (e.g: in .ports) that match parameters
	...
	func delayedPopulate(named wantName:String?=nil, localUp wantUp:Bool?=nil) -> Port? {
...
//	}
	...
	override func partMatching(path:Path) -> Part? {
		...
		guard let proxy			= super.partMatching(path:path) else {
	...
	override func gatherLinkUps(into linkUpList:inout [() -> ()]) {    //super gatherWiresInto:wirelist];
		...
		for (srcPortString, targets_) in localConfig {
			...
			for var trgAny : FwAny in targets_ as? [FwAny] ?? [targets_] {
				...
				root.wireNumber += 1
				let wireNumber	= root.wireNumber
				 // Break at Wire?
...
/* **************************************************************************/
					...
					assert(trgPort != nil, "Couldn't find \(trgInfo)")

					 //    3b. SouRCe (self)
					let trgAboveSInS = trgAboveSInCon ^^ self.upInPart(until:conNet)
					let srcInfo	= "L\(wireNumber)'s SOURCE:\(self.fullName16)" +
					...
					assert(srcPort != nil, "Couldn't find \(srcInfo)")
					...
					var msg 	= "L\(wireNumber) ADDED: << \(srcPort?.fullName ?? "") "
					...
					atBld(4, self.logd(msg + "<-> \(trgPort!.fullName) >> in:\(conNet.fullName)"))

					 // CHECK: Boss and Worker Ports face opposite
					if trgPort!.upInWorld ^^ !srcPort!.upInWorld,	// direction fault
					  !(self.check("noCheck", in:neighbors) == true)	// not noCheck
					{	msg		=  "\n  Within Containing Net" + conNet.fullName + ":"
						msg		+= "\n\t" + "Source: " + srcPort!.fullName
						msg		+= " " + srcPort!.upInWorldStr()
						msg    	+= "\n\t" + "Target: " + trgPort!.fullName
						msg		+= " " + trgPort!.upInWorldStr()
						self.warning("Attempt to link 2 Ports both with worldDown=\(srcPort!.upInWorldStr())." +
								" Consider using config[noCheck] '^'." + msg)
					}
					...
					if link==nil {
						...
						conNet.addChild(link, atIndex:lnkInsInd)
						 // Link may have active segments from creation
...
//	}
	...
	override func reSize(inVew vew:Vew) {
		vew.children.forEach {$0.keep = false}	// mark all Views as unused
				...
				// 1. Resizes all  _CHILD Atoms_ FIRST (inside inner box)
		super.reSize(inVew:vew)

				// 2. reSize  _CHILD Ports_ around the packed Atoms
		if vew.expose == .open {
			var tmpBBox		= vew.bBox			// Loop without changing vew.bBox
			for (portName, port) in ports {		// FOR ALL Ports, if a Vew exists:
				if let portVew	= vew.find(name:"_" + portName, maxLevel:1) {
					portVew.keep = true
					if port.testNReset(dirty:.size) {
						port.reSize(inVew:portVew) // 2A. Pack Port
					}
					rePosition(portVew:portVew)	// 2B.Reposition Port via Atom:
					let bBoxInAtom = portVew.bBox * portVew.scn.transform
					tmpBBox	|= bBoxInAtom		// Accumulate into _tmpBBox_
				}
			}
			vew.bBox		= tmpBBox	 		// Install temporary BBox
		}
			  ...
			  fixedPort.dominant || fixedPort.connectedTo!.dominant
			{
				...
				for key in ["type", "t"] {
					if let any  	= theLink?.localConfig[key] {
						...
						assertWarn(dla != nil, "Configuration 'linkSkinType' has illegal value '\(any.pp())'")
