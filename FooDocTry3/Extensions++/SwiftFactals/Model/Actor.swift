...
//	var previousClocksEnable : Port
 //	var timingChain :
	...
	enum ActorKeys: String, CodingKey {
		case con, evi, previousClocks, positionViaCon, viewAsAtom_, linkDisplayInvisible
	}
	 // Serialize
	...
	override func copy(with zone: NSZone?=nil) -> Any {
	 ...
	 // MARK: - 3.7 Equitable
	func varsOfActorEq(_ rhs:Part) -> Bool {
		guard let rhsAsActor	= rhs as? Actor else {	return false		}
		return con     		== rhsAsActor.con
			&& evi			== rhsAsActor.evi
			&& previousClocks == rhsAsActor.previousClocks
			&& positionViaCon == rhsAsActor.positionViaCon
			&& viewAsAtom_ 	  == rhsAsActor.viewAsAtom_
	}
	override func equalsPart(_ part:Part) -> Bool {
		return	super.equalsPart(part) && varsOfActorEq(part)
	}
...
//							let othersActorPart = otherAtom.ancestorThats(childOf:self)
							if let otherI = children.firstIndex(of:otherAtom),
							  otherI > scanI		 {
	...
	func enforceOrder() {
		if con != nil {			// ///// con exists:
			if let ind 			= children.firstIndex(of:con!),
			  ind != 0 {						  // con not first:
		...
		if evi != nil {			// ///// evi exists:
			if let ind 			= children.firstIndex(of:evi!),
