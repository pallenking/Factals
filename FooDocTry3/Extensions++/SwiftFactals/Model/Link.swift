...
//??	assert(sDownConvPort.dirty == .clean, "paranoia")						//sPort.dirty.turnOff(.vew) // (link vew proper at init)
	...
	override func hasPorts() -> [String:String]	 {  	["P":"cC", "S":"cCf"]	}
//	override func hasPorts() -> [String:String]	 {  	["P":"c", "S":"cf"]		}
//	override func hasPorts() -> [String:String]	 {  	[:]						}	//Ports defined elsewhere "["P":"c", "S":"cf"]		}
...
//		}
	...
	override func reVewPost(vew:Vew) 	{	// Add constraints
		...
		guard let p				= vewConnected(toPortNamed:"P", inViewHier:vew)
		 else {			fatalError("\nLink end \(self.fullName).P unconnected")	}
		guard let s				= vewConnected(toPortNamed:"S", inViewHier:vew)
...
//				linkVew.scn.constraints = [[], [con0], [con0, con1]][nConstraints]
		...
		return		// Xyzzy19e
		panic("\(self.fullName):Link end ('P' and/or 'S' Port) unconnected ")
	}
