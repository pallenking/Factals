...
}	//code Extension outside of file declaring struct 'CATransform3D' prevents automatic synthesis of 'encode(to:)' for protocol 'Encodable'
...
extension Array			: FwAny	where Element : Codable	{						}//extension Array : FwAny where Element : Codable {}
//
...
//}	// Extension outside of file declaring class 'SCNMaterial' prevents automatic synthesis of 'encode(to:)' for protocol 'Encodable'
...
extension FwAny  {
	...
	var asCGFloat 		  : CGFloat?{
		...
		if let s = self as? String	{	bug}//return CGFloat(s)						}
		return nil
	}
	...
	var asString 		  : String?	{
		...
		if let f = self as? FwConfig{	bug}//return f.pp(.line)						}
		return pp(.line)
	}
