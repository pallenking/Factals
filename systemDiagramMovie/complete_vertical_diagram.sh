...
DIAGRAM_FILE="../Docs/ApplicationViewControllerV.pages"
OUTPUT_DIR="./frames_vertical_chrono"
TEMP_DIR="./temp_vertical_chrono"
...
log "Setting up COMPLETE chronological extraction for ApplicationViewControllerV.pages..."
...
# Get ALL commits in chronological order (oldest first) for the V file
log "Getting ALL commits for ApplicationViewControllerV.pages in chronological order (oldest first)..."
commits=($(git log --oneline --follow --reverse "$DIAGRAM_FILE" | awk '{print $1}'))

total_commits=${#commits[@]}
log "Processing ALL $total_commits commits chronologically for vertical diagram..."
...
if ffmpeg -y -framerate 3 -pattern_type glob -i "$OUTPUT_DIR/*.png" \
           ...
           "system_evolution_vertical_complete.mp4"; then
    
    log "Movie created successfully: system_evolution_vertical_complete.mp4"
    ...
    # File size info
    movie_size=$(ls -lh "system_evolution_vertical_complete.mp4" | awk '{print $5}')
...
log "Complete chronological extraction for VERTICAL diagram completed!"
echo ""
echo "🎬 COMPLETE VERTICAL SYSTEM EVOLUTION MOVIE CREATED!"
echo "===================================================="
echo ""
echo "📁 Files created:"
echo "   Frames: $OUTPUT_DIR/ ($successful_extractions frames)"
echo "   Movie:  system_evolution_vertical_complete.mp4"
echo ""
...
echo ""
echo "⏱  This represents your complete VERTICAL diagram evolution timeline!"