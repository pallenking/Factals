...
DIAGRAM_FILE="Docs/SUI Diffing Diagram.pages"
OUTPUT_DIR="./frames_sui_chrono"
TEMP_DIR="./temp_sui_chrono"
...
log "Setting up chronological extraction for SUI Diffing Diagram.pages..."

# Setup directories
mkdir -p "$OUTPUT_DIR" "$TEMP_DIR"
rm -f "$OUTPUT_DIR"/* "$TEMP_DIR"/* 2>/dev/null || true

# Get ALL commits in chronological order (oldest first) for the SUI file
log "Getting ALL commits for SUI Diffing Diagram.pages in chronological order (oldest first)..."
commits=($(git log --oneline --follow --reverse "$DIAGRAM_FILE" | awk '{print $1}'))

total_commits=${#commits[@]}
log "Processing ALL $total_commits commits chronologically for SUI diffing diagram..."
...
for i in "${!commits[@]}"; do
    ...
    if unzip -q "$temp_file" -d "$extract_dir" 2>/dev/null && [ -f "$extract_dir/preview.jpg" ]; then
        ...
        # Add ONLY number and date/time in upper left corner - purple text, higher resolution (18pt)
        # NO "Commit" word as requested
        text_overlay="$((i+1))/$total_commits $commit_datetime"
        
        if command -v magick >/dev/null 2>&1; then
            magick "$output_file" \
                    -pointsize 18 -fill purple -stroke white -strokewidth 0.8 \
...
log "Creating chronological movie from $successful_extractions frames at 3 fps..."
if ffmpeg -y -framerate 3 -pattern_type glob -i "$OUTPUT_DIR/*.png" \
           -c:v libx264 -pix_fmt yuv420p \
           -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2" \
           "system_evolution_sui_complete.mp4"; then
    
    log "Movie created successfully: system_evolution_sui_complete.mp4"
    ...
    # File size info
    movie_size=$(ls -lh "system_evolution_sui_complete.mp4" | awk '{print $5}')
...
log "Complete chronological extraction for SUI Diffing diagram completed!"
echo ""
echo "🎬 COMPLETE SUI DIFFING DIAGRAM EVOLUTION MOVIE CREATED!"
echo "========================================================"
echo ""
echo "📁 Files created:"
echo "   Frames: $OUTPUT_DIR/ ($successful_extractions frames)"
echo "   Movie:  system_evolution_sui_complete.mp4"
echo ""
...
echo "   • Purple text: 'N/Total YYYY-MM-DDTHH:MM:SS' (NO 'Commit' word)"
echo "   • Higher resolution font (18pt)"
echo "   • Chronological order: oldest → newest"
echo ""
echo "⏱  This represents your complete SUI Diffing diagram evolution timeline!"