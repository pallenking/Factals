...
//		}
	...
	override func reVewPost(vew:Vew) 	{	// Add constraints
		...
		if linkVew.scnScene.constraints == nil {
			if linkSkinType == .dual {	 	// SCNBillboardConstraint, SCNLookAtConstraint
				...
				let con1 = SCNLookAtConstraint(target:s.scnScene) // :H: Number (of CONSTRAINTS)

				let nConstraints = 0		// debugging variant. use constraints, else do by hand
				linkVew.scnScene.constraints = [[], [con0], [con0, con1]][nConstraints]
			}
		}
	}
	...
	override func reSize(vew:Vew) {
		...
		vew.scnScene.categoryBitMask = FwNodeCategory.picable.rawValue 	// Link skins picable
...
//bug //NReset
			if port.test(dirty:.size) {		// clear Port's dirty size
				...
				portsVew.scnScene.categoryBitMask = FwNodeCategory.picable.rawValue 	// Link Port skins picable
			}
		}
	...
	override func reSkin(fullOnto vew:Vew) -> BBox  {
		...
		let name				= "s-Link"
		let _/*scn*/ : SCNNode	= vew.scnScene.find(name:name) ?? {
			let sLink			= SCNNode()
			sLink.name			= name
			vew.scnScene.addChild(node:sLink, atIndex:0)
	...
	override func reSizePost(vew:Vew) {				//  find endpoints
		...
		linkVew.scnScene.position	= .zero
...
bug	// Never USED?
	...
	override func computeLinkForces(vew:Vew) {
		...
		guard !vew.scnScene.transform.isNan else {
...
//			}
	...
	/**/		sInertialVew?.force -= force
				...
				atAni(9, logd(" posn: \(vew.scnScene.transform.pp(.line))"))
			}
			else {
...
//			atRsi(8, logd("<><> L 9.5.4: \\set xform from p:\(pEndVip.pp(.line)) s:\(sEndVip.pp(.line))"))
			...
			let s				= linkVew.scnScene.find(name:"s-Link")!
			s.transform 		= m
		}else{
	...
	override func rePaint(vew:Vew) 		{		// paint red and green
		...
		if linkSkinType == .dual {
			...
			let scn2paintOn	= linkVew.scnScene.find(name: "s-Paint")
		...
		if let scnLink:SCNNode = vew.scnScene.find(name:"s-Link") {
...
extension Port {
	...
	func reSkin(linkPortsVew vew:Vew) -> BBox  {
		...
		let name				= "s-LinkEnd"
		let scn:SCNNode 		= vew.scnScene.find(name:name) ?? {
			 // None found, make one.
			...
			vew.scnScene.addChild(node:scn)
			scn.name			= name
