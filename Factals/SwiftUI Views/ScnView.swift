...
bug
		...
		let ratioHigher			= frame.height / frame.width
		if rootVewSizeInEye.y > rootVewSizeInEye.x * ratioHigher {
			zoomRv				*= ratioHigher
		}
		if rootVewSizeInEye.x * frame.height < frame.width * rootVewSizeInEye.y {
...
//			motorSpinNUp(with:nsEvent)			// change Spin and Up of camera
		...
		case .beginGesture:	bug	// override func touchesBegan(with event:NSEvent) {
			let touchs			= nsEvent.touches(matching:.began, in:self)
			for touch in touchs {
				let _:CGPoint	= touch.location(in:nil)
			}
		case .mouseMoved: bug
			let touchs			= nsEvent.touches(matching:.moved, in:self)
			for touch in touchs {
		...
		case .endGesture: bug	//override func touchesEnded(with event:NSEvent) {
			let touchs			= nsEvent.touches(matching:.ended, in:self)
			for touch in touchs {
				let _:CGPoint	= touch.location(in:nil)
			}
		default:
...
/**/	let hits 				= hitTest(locationInRoot, options:configHitTest)
		...
		var msg					= "******************************************\n Slot\(vewBase.slot_): "
		msg 					+= "found \(pickedScn.pp(.classTag)): \(pickedScn.fullName)"
